<div align="center">
  ...
  <h1>HiEQ · HOME</h1>
  <p><b>单文件个人主页</b> ＋ <b>Die Shot 裸片显微图库</b></p>
  <p>Live2D 桌宠 · 液态玻璃 UI · 夜间模式 · 中英双语 · 移动端自适应</p>
  <p>
...
这是一个**单文件静态站**：`index.html` 里装下了页面、样式、交互、多语言与管理后台，没有任何构建步骤、没有前端框架 —— 打开就能跑，推一下就上线。

站点分三个部分：

| 版块 | 内容 |
| --- | --- |
| 🏠 **首页** | 整屏实景背景 + 「HIEQ的小站」液态玻璃标题与跳转按钮 + Live2D 桌宠 |
| 🔬 **Layout** | **Die Shot 裸片显微图库**：221 张芯片裸片影像，27 家厂商，支持搜索与标签筛选 |
...
### 🐱 Live2D 桌宠
- **双模型**：`HEiQ` 与 `NULL`，可在编辑模式里一键切换（切换时位置、大小自动保留）
- **表情系统**：每个模型各自的表情集（HEiQ 6 个 / NULL 7 个），数量不同也能自动适配
- **鼠标拖拽**：编辑模式下直接拖动调整位置，电脑端 / 移动端**各存一份、互不影响**
- **大小可调**：50%–180% 滑杆缩放，画布与模型同步重算，放大不糊
- **眨眼联动**：非默认表情时彻底停眨眼（不会「眨眼叠在表情上」），回到默认表情才恢复
- **真随机表情**：点按桌宠或每 8 秒随机换一张脸（等概率抽取、不重复连抽）
- **贴边折叠**：任何位置、任何尺寸下都能一键藏到屏幕边缘，只留侧边箭头
- **只存本机**：位置 / 大小 / 表情 / 随机开关全部存在浏览器本地，改桌宠不会触发仓库提交与重建

### 🖼️ Die Shot 图库
- **221 张**裸片显微影像，网页版统一 **4096px 长边 / JPEG q45**（约 342 MB），原图约 9.4 GB 留在本地
...
```jsonc
{
  ...
  "config": {
    "adminHash": "管理口令的 SHA-256",   // Token 绝不入库，只存在浏览器本地
